 ...
 * public_key: Uint8Array (1184 bytes, base64url-decoded server public key)
 * Returns { ciphertext: Uint8Array, shared_secret: Uint8Array } or throws.
 * @param {Uint8Array} public_key
 * @returns {object}
 */
export function encaps(public_key) {
    const ptr0 = passArray8ToWasm0(public_key, wasm.__wbindgen_malloc);
    const len0 = WASM_VECTOR_LEN;
    const ret = wasm.encaps(ptr0, len0);
    if (ret[2]) {
 ...
 * shared_secret: Uint8Array (32 bytes from encaps)
 * token: string
 * Returns claims as a JSON string, or throws on failure.
 * @param {Uint8Array} shared_secret
 * @param {string} token
 * @returns {string}
 */
export function verify_paseto(shared_secret, token) {
    let deferred4_0;
    let deferred4_1;
    try {
        ...
        const ret = wasm.verify_paseto(ptr0, len0, ptr1, len1);
        var ptr3 = ret[0];
        var len3 = ret[1];
        if (ret[3]) {
            ptr3 = 0; len3 = 0;
